...
void bounce_ball(Pong* pong)
{
    if (pong->ball.x - pong->ball.radius < 50) {
        int rel_y = pong->ball.y - pong->left_pad.y;
        if (0 <= rel_y && rel_y <= pong->left_pad.height) {
    ...
    if (pong->ball.x + pong->ball.radius > pong->width - 50) {
        int rel_y = pong->ball.y - pong->right_pad.y;
        if (0 <= rel_y && rel_y <= pong->right_pad.height) {
